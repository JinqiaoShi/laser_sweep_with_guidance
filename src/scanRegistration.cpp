...
void imuHandler(const sensor_msgs::Imu::ConstPtr& imuIn)
{
 ...
 // float accX = (imuIn->linear_acceleration.y - sin(yaw) * cos(pitch))*9.8;
 // float accY = (-imuIn->linear_acceleration.z-1 + cos(yaw) * cos(pitch))*9.8;
 // float accZ = (imuIn->linear_acceleration.x + sin(pitch))*9.8;

    float accX = -imuIn->linear_acceleration.y*9.8;
float accY = 0;
float accZ = imuIn->linear_acceleration.x*9.8;
